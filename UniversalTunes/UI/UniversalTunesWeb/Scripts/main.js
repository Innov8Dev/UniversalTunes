$(function () {
    //Query string that loads on the url for searching
    $(".tabicontainer input[type=text]").val(location.hash.substring(0)).trigger("keyup").keyup(function () { location.hash = $(this).val(); });
    ...
    /*
    JHNet - Beni 
    This code below does it for all items in teh UL Quick Actions
    I have changed it to the one below to only toggle the item called sidebar
    ...
    $('ul.quick-actions > li > a.sidebar').click(function () {
        var element = $(this).parents('li')
        if (element.hasClass('active')) {
            element.removeClass('active');
        } else {
