angular
    .module('app.EnumService', [])
    .service('EnumService', EnumService);

EnumService.$inject = ['$http', '$rootScope', '$q'];

function EnumService($http, $rootScope, $q) {
    ...
    function loadSecurityType() {
        var defered = $q.defer();
        return $http.get("/api/AspNetRoles/roles")
            .then(function (result) {
    ...
    function priceType() {
        var defered = $q.defer();
        return $http.get("/api/Enums/PriceType")
            .then(function (result) {
    ...
    function checkAllEnumsLoaded() {
        if (vm.securityType.length > 0 && vm.userType.length > 0 && vm.priceType.length > 0) {
